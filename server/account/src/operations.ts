...
export async function validateOtp (
  ...
  if (account == null) {
    // This is a signup
    await createAccount(db, emailSocialId.personId)
...
export async function createInviteLink (
  ...
  // TODO: do we need to restore personId restriction in links???
...
// TODO: personId not supported! Check why it's needed?
...
export async function selectWorkspace (
  ...
  const { account: accountUuid, workspace: workspaceUuid, extra } = decodeTokenVerbose(ctx, token)
  ...
  if (accountUuid === GUEST_ACCOUNT && extra?.guest === 'true') {
    ...
    return {
      account: accountUuid,
      endpoint: getEndpoint(ctx, workspace.uuid, workspace.region, endpointKind),
      token,
      workspace: workspace.uuid,
      workspaceUrl: workspace.url,
      role: AccountRole.Guest // TODO: is it the correct role?
    }
  }
  ...
  if (workspaceUrl === '') {
    // Find from token
    workspace = await getWorkspaceById(db, workspaceUuid)
  } else {
  ...
  if (accountUuid === systemAccountUuid) {
    return {
      account: accountUuid,
      token: generateToken(accountUuid, workspaceUuid),
      endpoint: getEndpoint(ctx, workspace.uuid, workspace.region, endpointKind),
      workspace: workspace.uuid,
      workspaceUrl: workspace.url,
      role: AccountRole.Owner
  ...
  if (wsStatus != null) {
    if (wsStatus.isDisabled && wsStatus.mode === 'active') {
...
export async function join (
  ...
  if (role == null) {
    await db.assignWorkspace(account, workspaceUuid, invite.role)
  } else if (getRolePower(role) < getRolePower(invite.role)) {
    await db.updateWorkspaceRole(account, workspaceUuid, invite.role)
  }
...
export async function signUpJoin (
  ...
  const token = generateToken(account, workspaceUuid)
  const result = await selectWorkspace(ctx, db, branding, token, workspace.url, 'external')

  await useInvite(db, inviteId)

  ctx.info('Successfully signed up and joined a workspace using invite', {
    normalizedEmail,
    workspaceUuid,
    workspaceName: workspace.name
  })
...
export async function requestPasswordReset (
  ...
  // TODO: do not throw but confirm in password reset handle
  if (emailSocialId.verifiedOn == null) {
    ctx.error('Email social id not confirmed', { ...emailSocialId, email, normalizedEmail })
    throw new PlatformError(
      new Status(Severity.ERROR, platform.status.SocialIdNotConfirmed, { socialId: email, type: SocialIdType.EMAIL })
    )
  }
...
export async function getWorkspaceInfo (
  ...
  if (workspace == null || workspace.status.isDisabled || workspace.status.mode !== 'active') {
...
export async function getLoginInfoByToken (
  ...
  try {
    ;({ account: accountUuid, workspace: workspaceUuid } = decodeTokenVerbose(ctx, token))
  } catch (err: any) {
    Analytics.handleError(err)
