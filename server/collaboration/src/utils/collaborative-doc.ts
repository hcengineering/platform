...
async function loadCollaborativeDocVersion (
  ...
  const yContent = await ctx.with('yDocFromStorage', { type: 'content' }, async () => {
...
export async function takeCollaborativeDocSnapshot (
  ...
  await ctx.with('takeCollaborativeDocSnapshot', {}, async (ctx) => {
    const yHistory =
      (await ctx.with('yDocFromStorage', { type: 'history' }, async () => {
    ...
    await ctx.with('yDocToStorage', { type: 'history' }, async () => {
