...
test.describe('channel tests', () => {
  ...
  test('create new private channel and test if the user can exchange the messages', async ({ browser, page }) => {
    ...
    await channelPageSecond.checkIfChannelDefaultExist(false, data.channelName)
    await channelPageSecond.clickChannelTab()
    await attachScreenshot('Chat_Channel-channels.png', page2)
    await channelPageSecond.checkIfChannelTableExist(data.channelName, true)
    ...
    const joinButton = await page2.locator('button[data-id="btnJoin"]').isVisible({ timeout: 1500 })
    if (joinButton) {
      await attachScreenshot('Chat_Channel-hasJoin.png', page2)
      await page2.locator('button[data-id="btnJoin"]').click()
    }
    await attachScreenshot('Chat_Channel-searchTestMsg.png', page2)
