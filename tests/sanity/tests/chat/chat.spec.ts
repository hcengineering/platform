...
test.describe('channel tests', () => {
  ...
  test.only('User can create direct chat with employee', async ({ request, page, browser }) => {
    const page2 = await browser.newPage()
    const channelPageSecond = new ChannelPage(page2)
    const leftSideMenuPageSecond = new LeftSideMenuPage(page2)
    ...
    await test.step('Create a direct chat', async () => {
      await leftSideMenuPage.clickChunter()
      await chunterPage.clickChannelBrowser()
      await chunterPage.clickNewDirectChatHeader()
      await chunterPage.page.locator('.popup input[placeholder="Search..."]').fill(`${newUser2.lastName}`)
      await chunterPage.page
        .locator('.popup .users button.row')
        .filter({ hasText: `${newUser2.lastName} ${newUser2.firstName}` })
        .click()
      await chunterPage.page.locator('.hulyModal-footer button:has-text("Next")').click()
      await chunterPage.page.locator('.hulyModal-footer button:has-text("Create")').click()
      await expect(channelPage.page.locator('.hulyNavGroup-header:has-text("Direct messages")')).toBeVisible()
    })
    ...
    await test.step('Exchange messages in the direct chat', async () => {
      ...
      await channelPage.sendMessage('Test direct message')

      await channelPageSecond.clickChooseChannel(`${data.lastName} ${data.firstName}`)
      await channelPageSecond.checkMessageExist('Test direct message', true, 'Test direct message')
