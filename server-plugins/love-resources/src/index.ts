...
async function createUserInfo (user: Ref<Person>, control: TriggerControl): Promise<Tx[]> {
  // we already have participantInfo for this person
  const infos = await control.findAll(control.ctx, love.class.ParticipantInfo, { person: user })
  if (infos.length > 0) return []

  const person = (await control.findAll(control.ctx, contact.class.Person, { _id: user }))[0]
  const room = (await control.findAll(control.ctx, love.class.Office, { person: user }))[0]
...
async function removeUserInfo (user: Ref<Person>, control: TriggerControl): Promise<Tx[]> {
...
export async function OnUserStatus (txes: Tx[], control: TriggerControl): Promise<Tx[]> {
  ...
  for (const tx of txes) {
    ...
    if (actualTx._class === core.class.TxCreateDoc) {
      ...
      result.push(...(await createUserInfo(status.user as Ref<Person>, control)))
      ...
      if (status !== undefined) {
        if (val) {
          ...
          result.push(...(await removeUserInfo(status.user as Ref<Person>, control)))
        }
      }
    }
  }
