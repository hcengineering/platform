...
export abstract class TxProcessor implements WithTx {
  ...
  protected txCollectionCUD (tx: TxCollectionCUD<Doc, AttachedDoc>): Promise<TxResult> {
    ...
    for (const childTx of txes) {
      if (childTx._class === core.class.TxCreateDoc) {
        const d = (childTx as TxCreateDoc<AttachedDoc>)
        d.attributes = { ...d.attributes, attachedTo: tx.objectId, attachedToClass: tx.objectClass }
      }
    }

    const bulk: TxBulkWrite = { ...data, txes }
    return this.txBulkWrite(bulk)
  }
...
export class TxOperations implements Storage {
  ...
  async addCollection<T extends Doc, P extends AttachedDoc> (
    ...
    const childTx = this.txFactory.createTxCreateDoc<P>(_class, space, attributes as unknown as Data<P>, id)
    const tx = this.txFactory.createTxCollectionCUD<T, P>(attachedToClass, attachedTo, space, collection, [childTx])
    await this.storage.tx(tx)
    return tx.objectId
  }
  ...
  async updateCollection<T extends Doc, P extends AttachedDoc> (
    ...
    const childTx = this.txFactory.createTxUpdateDoc(_class, space, objectId, operations)
    const tx = this.txFactory.createTxCollectionCUD(attachedToClass, attachedTo, space, collection, [childTx])
    await this.storage.tx(tx)
    return tx.objectId
  }
...
export class TxFactory {
  ...
  createTxCollectionCUD<T extends Doc, P extends AttachedDoc>(_class: Ref<Class<T>>, objectId: Ref<T>, space: Ref<Space>, collection: string, txes: TxCUD<P>[]): TxCollectionCUD<T, P> {
    return {
      _id: generateId(),
