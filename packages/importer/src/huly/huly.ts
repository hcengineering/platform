...
export interface HulyIssueHeader {
  ...
  // Schedule fields surfaced in the Gantt viewlet.
  // Dates are ISO YYYY-MM-DD; the importer treats them as UTC midnight.
  startDate?: string
  dueDate?: string
  // Independent soft deadline. Renders a flag marker in
  ...
  component?: string
  milestone?: string
  // Finish-to-Start dependency list, by predecessor issue
...
export interface HulyProjectSettings extends HulySpaceSettings {
  ...
  // Declarative components + milestones on the project.
...
export class HulyFormatImporter {
    ...
    for (const issueFile of issueFiles) {
      ...
      if (issueHeader.class === tracker.class.Issue) {
        ...
        const parseIsoDate = (iso?: string): number | undefined => {
          ...
          // Relative-date placeholders for the Game Design
  ...
  private async processProject (data: HulyProjectSettings): Promise<ImportProject> {
    ...
    return {
      ...
      docs: [],
      // Declarative components + milestones
