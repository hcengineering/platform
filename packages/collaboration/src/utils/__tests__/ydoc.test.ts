...
import { yDocCopy, yDocCopyWithGC, yDocFromBuffer, yDocToBuffer } from '../ydoc'
...
describe('ydoc', () => {
  ...
  describe('yDocCopy', () => {
    it('copies document state without gc', async () => {
      const source = new YDoc({ gc: false })

      applyGarbageCollectableChanges(source)

      const target = yDocCopy(source)
    ...
    it('copies document state with gc', async () => {
  ...
  describe('yDocCopyWithGC', () => {
    it('copies document state without gc', async () => {
      const source = new YDoc({ gc: false })

      applyGarbageCollectableChanges(source)

      const target = yDocCopyWithGC(source)
