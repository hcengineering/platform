...
describe('sidebar-columns: parseWidths', () => {
  it('returns the defaults for undefined / null / non-objects / arrays', () => {
    expect(parseWidths(undefined)).toEqual(DEFAULT_WIDTHS)
    expect(parseWidths(null)).toEqual(DEFAULT_WIDTHS)
    expect(parseWidths('slack')).toEqual(DEFAULT_WIDTHS)
    expect(parseWidths([['slack', 200]])).toEqual(DEFAULT_WIDTHS)
  })

  it('overrides only the supplied columns and keeps defaults for the rest', () => {
    const out = parseWidths({ slack: 120, title: 300 })
    expect(out.slack).toBe(120)
    expect(out.title).toBe(300)
    expect(out.identifier).toBe(DEFAULT_WIDTHS.identifier)
    expect(Object.keys(out).sort()).toEqual([...ALL_COLUMN_KEYS].sort())
  })

  it('drops unknown keys and non-numeric values', () => {
    const out = parseWidths({ nope: 500, slack: 'wide', predecessors: Number.NaN })
    expect(out).toEqual(DEFAULT_WIDTHS)
  })

  it('clamps out-of-range widths', () => {
    const out = parseWidths({ slack: -50, title: 100_000 })
    expect(out.slack).toBe(MIN_WIDTH)
    expect(out.title).toBe(MAX_WIDTH)
  })

  it('round-trips a persisted width through computeTotalWidth', () => {
    const cols: SidebarColumnKey[] = ['identifier', 'slack']
    const widths = parseWidths({ slack: 111 })
    expect(computeTotalWidth(cols, widths)).toBe(DEFAULT_WIDTHS.identifier + 111)
  })
})
