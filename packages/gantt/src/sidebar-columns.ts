...
/** Union of all built-in column keys. */
...
/**
 * Coerce an unknown ViewOption value into a per-column width map.
 *
 * Column widths are persisted as a ViewOption so a resized Slack /
 * Predecessors / Title column survives a viewlet re-mount instead of
 * snapping back to {@link DEFAULT_WIDTHS}. Same defensive contract as
 * {@link parseColumns}: unknown keys and non-finite / out-of-range values
 * are dropped, everything else is clamped, and the result always starts from
...
export function parseWidths (raw: unknown): Record<SidebarColumnKey, number> {
  ...
  for (const [key, value] of Object.entries(raw as Record<string, unknown>)) {
    if (!KNOWN_SET.has(key)) continue
    if (typeof value !== 'number' || !Number.isFinite(value)) continue
    out[key as SidebarColumnKey] = clampWidth(value)
  }
  return out
}
