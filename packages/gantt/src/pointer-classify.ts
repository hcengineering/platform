 ...
 * full-edit-on-tablet / read-only-on-phone decision. Keeping it as a pure
...
export function classifyPointer (mode: LayoutMode, pointerType: PointerKind, action: PointerAction): PointerDecision {
  ...
  // confirmation (a 300 ms long-press enters selection mode), so a plain
  // touch stays available for scrolling.
