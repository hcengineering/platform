...
import serverClient, { getWorkspaceInfo } from '@hcengineering/server-client'
import {
...
import model from '@hcengineering/model-all/bundle/model.json'
...
export class Transactor extends DurableObject<Env> {
  ...
  private workspaceInfo: BaseWorkspaceInfo | undefined
  ...
  constructor (ctx: DurableObjectState, env: Env) {
    ...
    void this.ctx.blockConcurrencyWhile(async () => {
      const token = generateToken(systemAccountEmail, getWorkspaceId(this.workspace))
      setMetadata(serverClient.metadata.Endpoint, env.ACCOUNTS_URL)

      this.workspaceInfo = await getWorkspaceInfo(token)
      if (this.workspaceInfo === undefined) {
        return
      }
