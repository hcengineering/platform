...
for en_file in "${en_json_files[@]}"; do
  ...
  # Dosyayı kopyala (en.json içeriğini es.json'a)
  cp "$en_file" "$es_file"

  # İşlem mesajı (isteğe bağlı)
  echo "Kopyalandı: $en_file -> $es_file"
done

echo "Tüm en.json dosyaları es.json olarak kopyalandı."