...
<script lang="ts">
  ...
  $: badge = $intercomStore.unreadCount > 0

  $: updateIntercom(currentAccount, workspaceId, language)
...
<button
  class="app {size}"
  class:selected
  use:tooltip={{ label: support.string.ContactUs }}
  on:click={toggleIntercomMessenger}
>
  <div class="flex-center icon-container" class:noty={badge}>
    <Icon icon={support.icon.Support} size={size === 'small' ? 'small' : 'medium'} />
  </div>
  {#if badge}<div class="badge" />{/if}
</button>
