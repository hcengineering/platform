...
function getLastVisitDays (it: WorkspaceInfoWithStatus): number {
  return Math.floor((Date.now() - it.lastVisit) / (1000 * 3600 * 24))
}

function getWorkspaceSize (it: WorkspaceInfoWithStatus): number {
  let sz = 0
...
export async function sendInvite (email: string, personId?: Ref<Doc>, role?: AccountRole): Promise<void> {
  await getAccountClient().sendInvite(email, personId, role)
}
