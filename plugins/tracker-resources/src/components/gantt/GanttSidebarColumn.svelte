...
<script lang="ts">
  /**
   * Per-column renderer. Reads a column key + a LayoutRow and
   ...
   * Click-to-edit is deliberately deferred until it can land together with
   * the undo-manager, so a missed edit cannot leak through with no rollback
   * path.
   *
  ...
  import type { SidebarColumnKey } from '@hcengineering/gantt'
  ...
  // Reactive closure, NOT a plain function: `{slackDays(issue._id)}d` would
  // otherwise only depend on `issue`, never on `slack`, and the debounced
  // critical-path pass delivers its Map long after the row was rendered — the
  // cell would be stuck on the initial "0d". Same trap as `slackDaysFor` in
