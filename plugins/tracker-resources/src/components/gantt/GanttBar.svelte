...
<script lang="ts">
  ...
  import type { TimeScale, PointerKind } from '@hcengineering/gantt'
  import type { DragState, DragTarget } from './lib/types'
  import GanttConnectorDot from './GanttConnectorDot.svelte'
  import { activeDragTargetId, classifyPointer, LONG_PRESS_MS, MOVE_THRESHOLD_PX } from '@hcengineering/gantt'
  ...
  // GanttBarIssueLike has all bar-data fields optional. Real Issues and
  ...
  // Edit-mode props. dragTarget carries the discriminated drag subject
  // (Issue or Milestone); editable gates body selection
  ...
  // multi-selection. Paints the same 2-px solid outline
  ...
  // → no glyph, which is the pre-existing default for every issue.
  ...
  // through a long-press timer on tablets. Default 'desktop'
  ...
  // Configurable bar-label slots driven by ViewOptions.
  ...
  // Edit-mode: while THIS bar is the active drag target, swap the bar
  ...
  // DragState carries `target: { kind, doc }` (Issue or Milestone)
  ...
  // Dates are day-granular and both ends are inclusive, so a bar that starts
  // and ends on the same day still spans one full day column.
  $: w = Math.max(2, x2 - x + timeScale.pxPerDay)
  ...
  $: tooltipText = visible
    ...
    : ''
  // Milestone-synthetic-summary claws were dropped (the milestone
  ...
  // before `issueObj` was renamed to `dragTarget`.
...
{#if visible}
  ...
  {:else}
    ...
    {#if isCritical && showSlackGlyph}
      <!-- 18%-opacity red overlay on critical bars, on top of the
    ...
    {#if slackPx > 0 && !isCritical}
      <!-- Slack glyph — light-grey trailing bar showing total
      ...
      />
    {/if}
    <!-- Resize-handle rendering moved to a sibling overlay layer in
         GanttCanvas (`<GanttBarResizeHandles>`, rendered AFTER
         GanttDependencyLayer) so the handles sit ABOVE the dep-arrows'
         invisible 12 px hit-stroke. Keeping them here meant every arrow
         that merely PASSED OVER a foreign bar swallowed the pointerdown
         aimed at that bar's handle and the chart panned instead of
         resizing. Same reasoning — and the same overlay layer pattern —
         as the connector dot below. -->
    <!-- Connector-dot rendering moved to a sibling overlay layer in
