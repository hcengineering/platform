...
<script lang="ts">
   ...
   * the popup-mount, mirroring how GanttView owns selectedIssueId.
   */
  ...
  // red dashed with '!' hint via title tooltip. Violated
  ...
  // Click-target geometry: the same curve with its two ends cut back so it
  // stops short of the bar edges. Dependency endpoints sit exactly on the bar
  // edges, which is also where GanttBar puts its 6px resize handles — and the
  // dependency layer paints after the bars, so an end-to-end hit-stroke made
  // the resize handles of every bar with a dependency unclickable. Trimming
  // keeps the arrow itself selectable (curve middle + lag pill) instead of
  // dropping its interactivity, which is a real feature: click opens the
  // dependency editor.
  $: hitPath = p1 !== null && p2 !== null ? bezierHitPath(p1, p2) : null
...
{#if path !== null && head !== null && mid !== null}
  <g class="gantt-dep-arrow" class:dimmed on:mouseenter={onEnter} on:mouseleave={onLeave}>
    <!-- Invisible wider stroke for an easier click target. Uses the TRIMMED
         geometry so the bar resize handles at both endpoints stay reachable
         (see `hitPath` above). -->
    <path d={hitPath} stroke="transparent" stroke-width={12} fill="none" on:click={onOpen} />
    <path
      d={path}
      class="curve"
