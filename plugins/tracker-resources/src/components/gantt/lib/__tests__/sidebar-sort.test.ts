...
import { cycleSort, comparatorFor, parseSortState, NO_SORT, type GanttSortState } from '../sidebar-sort'
...
describe('sidebar-sort: parseSortState', () => {
  it('falls back to NO_SORT for anything unrecognised', () => {
    expect(parseSortState(undefined)).toEqual(NO_SORT)
    expect(parseSortState(null)).toEqual(NO_SORT)
    expect(parseSortState('title')).toEqual(NO_SORT)
    expect(parseSortState([])).toEqual(NO_SORT)
    expect(parseSortState({ column: 'not-a-column', direction: 'asc' })).toEqual(NO_SORT)
    expect(parseSortState({ column: null, direction: 'desc' })).toEqual(NO_SORT)
  })

  it('restores a persisted column + direction', () => {
    expect(parseSortState({ column: 'slack', direction: 'desc' })).toEqual({ column: 'slack', direction: 'desc' })
    expect(parseSortState({ column: 'title', direction: 'asc' })).toEqual({ column: 'title', direction: 'asc' })
  })

  it('defaults a missing / bogus direction to asc', () => {
    expect(parseSortState({ column: 'title' })).toEqual({ column: 'title', direction: 'asc' })
    expect(parseSortState({ column: 'title', direction: 'sideways' })).toEqual({ column: 'title', direction: 'asc' })
  })

  it('round-trips whatever cycleSort produces', () => {
    let state: GanttSortState = { ...NO_SORT }
    for (let i = 0; i < 4; i++) {
      state = cycleSort(state, 'slack')
      expect(parseSortState(JSON.parse(JSON.stringify(state)))).toEqual(state)
    }
  })

  it('never hands back a shared NO_SORT instance', () => {
    const a = parseSortState(undefined)
    const b = parseSortState(undefined)
    expect(a).not.toBe(b)
    expect(a).not.toBe(NO_SORT)
  })
})
