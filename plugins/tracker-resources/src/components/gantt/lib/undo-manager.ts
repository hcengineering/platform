...
/**
 * Reserved for future inline attribute edits (via Quick-Info
    ...
    switch (entry.kind) {
      ...
      case 'date-batch': {
        for (const c of entry.changes) {
          ...
          // Partial failure is tolerated: an issue deleted since the entry
          // was recorded is skipped so the rest of the batch still reverts.
