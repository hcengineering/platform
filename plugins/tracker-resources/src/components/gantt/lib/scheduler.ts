...
/**
 * Cycle-detection for the IssueRelation graph. Returns true iff adding
  ...
  while (queue.length > 0) {
    ...
    for (const r of outgoing) {
      ...
      if (requiredAnchor > targetAnchor) {
        ...
        // original A→B gap. Auto-tightening like that is never what the user
        // asked for — a drag must not silently rewrite slack it did not
        // touch. The snap term still wins when the
