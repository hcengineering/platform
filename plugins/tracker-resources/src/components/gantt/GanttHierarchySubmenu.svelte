  ...
  const actions: UiAction[] = [
    {
      ...
      action: async () => {
        dispatch('close')
        // Opt into the resizable popup variant here only — the component
        // default ('large') stays untouched for all pre-existing callers.
        showPopup(SetParentIssueActionPopup, { value: issue, width: 'resizable' }, 'top')
      }
    },
    {
