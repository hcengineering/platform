...
<script lang="ts">
  ...
  $: visibleRange = [timeScale.fromX(Math.max(0, viewport.left - 100)), timeScale.fromX(viewport.right + 100)] as [
    number,
    number
  ]
...
<svg class="gantt-header" width={totalWidth} {height} viewBox="0 0 {totalWidth} {height}" preserveAspectRatio="none">
  {#each ticks as tick (tick.date)}
    ...
    />
    <text x={x + 4} y={height - 10} class="tick-label tick-label-{tick.level}" fill="var(--theme-content-color)">
      {tick.label}
    </text>
  {/each}
</svg>
