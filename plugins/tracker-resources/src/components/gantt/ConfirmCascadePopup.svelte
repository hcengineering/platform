...
<script lang="ts">
  ...
  /**
   * Cascade confirmation popup. Shows a mini-timeline with ghost
  ...
  function labelFor (i: Issue): string {
    ...
    // Unreachable in practice — every issue that reaches this popup carries
    // identifier + title (the cascade lookup projects both, see
    // `dragCommitProjection` in GanttView). Kept as a neutral PLACEHOLDER
    // rather than a raw `_id` fragment: a database id is meaningless to the
    // user and reads like a rendering bug.
    return '—'
  }
