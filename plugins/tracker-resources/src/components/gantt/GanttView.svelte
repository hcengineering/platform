...
<script lang="ts">
  ...
  import { createTimeScale, toGanttDay } from './lib/time-scale'
  ...
  $: issueQuery.query(
    ...
    issueDocQuery,
    (res: Issue[]) => {
      // M-G1: normalize date fields to UTC-midnight at the Gantt data boundary
  ...
  $: milestoneQuery.query(tracker.class.Milestone, milestoneDocQuery, (res: Milestone[]) => {
    // M-G1: same UTC-midnight boundary normalization as issues.
