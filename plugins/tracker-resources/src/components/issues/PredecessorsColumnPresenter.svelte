...
<script lang="ts">
   ...
   * Future optimisation (out of scope here): batched
  ...
  $: if (value !== undefined) {
    ...
    // Drop the live subscription when there is no value, otherwise a
  ...
  $: {
    ...
    if (sourceIds.length === 0) {
      // Same — release the previous source subscription when emptied.
