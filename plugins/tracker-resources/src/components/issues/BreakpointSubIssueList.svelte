  ...
  <svelte:fragment slot="item" let:item>
    ...
    >
      <div class="flex-row-center clear-mins gap-2 flex-grow mr-4" class:p-text={twoRows}>
        <FixedColumn key={'breakpoint_issue'} justify={'left'} addClass={'fs-bold'}>
          {issue.identifier}
        </FixedColumn>
      ...
      <div class="flex-row-center clear-mins gap-2 self-end flex-no-shrink" class:p-text={twoRows}>
        <FixedColumn key={'breakpoint_issue_assignee'} justify={'right'}>
          <AssigneeBox
            width={'100%'}
            kind={'ghost'}
        ...
        <FixedColumn key={'breakpoint'} justify={'left'}>
          <BreakpointEditor value={issue} kind={'list'} />
        </FixedColumn>
      </div>
    </div>
  </svelte:fragment>
