...
export async function connect (title: string): Promise<Client | undefined> {
  ...
  if (isWorkspaceCreating(workspace.mode)) {
    while (true) {
      ...
      workspaceCreating.set(workspace.processingProgress)
  ...
  const endpoint = workspaceLoginInfo?.endpoint
  ...
  const socialIds: SocialId[] = await getAccountClient().getSocialIds()
...
async function ensureEmployee (
  ...
  const person = await client.findOne(contact.class.Person, { personUuid: me.uuid })
  let personId: Ref<Person> | undefined = person?._id
  ...
  if (person === undefined) {
    await ctx.with('create-person', {}, async () => {
      ...
      const createPersonTx = txFactory.createTxCreateDoc(contact.class.Person, contact.space.Contacts, data, personId)

      await client.tx(createPersonTx)
    })
  }

  const employee = await client.findOne(contact.mixin.Employee, { _id: personId as Ref<Employee> })
  ...
  if (employee === undefined) {
    await ctx.with('create-employee', {}, async () => {
      ...
      const createEmployeeTx = txFactory.createTxMixin(personId, contact.class.Person, contact.space.Contacts, contact.mixin.Employee, {
        active: true
      })
  ...
  return personId as Ref<Employee>
}
