...
export function createWidgetTab (widget: Widget, tab: WidgetTab, newTab = false): void {
  ...
  if (devInfo.aside.float && !devInfo.aside.visible) { deviceInfo.set({ ...devInfo, aside: { visible: true, float: true } }) }
}
...
export function minimizeSidebar (closedByUser = false): void {
  ...
  const devInfo = get(deviceInfo)
  if (devInfo.navigator.float && devInfo.aside.visible) { deviceInfo.set({ ...devInfo, aside: { visible: false, float: true } }) }
}
