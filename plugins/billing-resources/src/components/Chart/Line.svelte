<script lang="ts">
  ...
  function createLines (data: { date: Date, value: number }[], width: number, height: number): { x1: number, y1: number, x2: number, y2: number }[] {
    const result: { x1: number, y1: number, x2: number, y2: number }[] = []
    const stepX = width / (data.length - 1)
    const minValue = Math.min.apply(Math, data.map((d) => d.value))
    const maxValue = Math.max.apply(Math, data.map((d) => d.value))
