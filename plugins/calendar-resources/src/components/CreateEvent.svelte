...
<script lang="ts">
  ...
  import core, { Class, Doc, Markup, Ref, Space, generateId, getCurrentPersonIds } from '@hcengineering/core'
  import presentation, {
    createQuery,
  ...
  $: myPrimaryId = $primarySocialIdByPersonRefStore.get(currentUser) ?? ''
