...
<script lang="ts">
  ...
  import { canViewTrainingSentRequests, getCurrentEmployeeRef } from '../utils'
  import NestedSpecialView from './NestedSpecialView.svelte'

  export let object: Training

  $: if (!canViewTrainingSentRequests(object)) {
...
<NestedSpecialView
  ...
  descriptors={[training.viewlet.TrainingSentRequests]}
/>
