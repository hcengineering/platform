<script lang="ts">
    ...
    $: cardQuery.query(task.class.State, { ...query, isArchived: true },
      result => { archivedLists = result },
...
{#if archivedLists}
    ...
    {#each archivedLists as list}
        <div class="w-full flex-row-center pt-2 pr-2 pb-2 pl-2">
            ...
            <div class="w-full">
                <Button width={'100%'} label={board.string.SendToBoard}
                    on:click={async () => { client.update(list, { isArchived: false }) }}
                />
            </div>
        </div>
