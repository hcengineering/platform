<script lang="ts">
  ...
  import { employeeByIdStore, socialIdsByPersonRefStore, statusByUserStore } from '../utils'
  ...
  $: socialIds = $socialIdsByPersonRefStore.get(employeeId) ?? []
  $: isOnline = socialIds.some((sid) => $statusByUserStore.get(sid.key)?.online === true)
