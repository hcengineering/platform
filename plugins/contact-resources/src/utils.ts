...
export const primarySocialIdByPersonRefStore = derived(
  ...
  (socialIdsByPersonRef) => {
    const mapped = Array.from(socialIdsByPersonRef.entries()).map(([_id, socialIds]) => [_id, pickPrimarySocialId(socialIds.map((si) => si.key))] as const)
    return new Map(mapped)
  }
)
/**
...
export const primarySocialIdByPersonIdStore = derived(
  ...
  (socialIdsByPersonId) => {
    const mapped = Array.from(socialIdsByPersonId.entries()).map(([personId, socialIds]) => [personId, pickPrimarySocialId(socialIds.map((si) => si.key))] as const)
    return new Map(mapped)
  }
)
