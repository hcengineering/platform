...
describe('document preview', () => {
  ...
  it('serializes conversions, coalesces duplicates and bounds the queue', async () => {
    ...
    const first = jest.fn(async () => await new Promise<string>((resolve) => { release = resolve }))
  ...
  it('releases failed jobs so retry is possible', async () => {
    ...
    await expect(queue.run('a', async () => { throw new Error('broken') })).rejects.toThrow('broken')
    await expect(queue.run('a', async () => 'retry')).resolves.toBe('retry')
  })

  it('sends document bytes and returns a validated PDF', async () => {
    const request = jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('%PDF-1.7\nfixture', {
  ...
  it.each([
    ...
    jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(body as string, {
      status: status as number, headers: { 'Content-Type': contentType as string }
    }))
  ...
  it('bounds both input and streamed output', async () => {
    const request = jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('%PDF-too-large', {
      headers: { 'Content-Type': 'application/pdf' }
    }))
    await expect(convertToPdf(Buffer.alloc(26 * 1024 * 1024), 'http://converter')).rejects.toMatchObject({ code: 413 })
    expect(request).not.toHaveBeenCalled()
    await expect(convertToPdf(Buffer.from('docx'), 'http://converter', { maxOutputBytes: 8 })).rejects.toMatchObject({ code: 502 })
  })

  it('aborts stalled conversions', async () => {
    jest.spyOn(globalThis, 'fetch').mockImplementation(async (_url, options) => await new Promise((_resolve, reject) => {
      options?.signal?.addEventListener('abort', () => reject(new Error('aborted')))
    }))
    await expect(convertToPdf(Buffer.from('docx'), 'http://converter', { timeoutMs: 10 })).rejects.toMatchObject({ code: 504 })
  })
})
