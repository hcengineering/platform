import { ArrOf, Class, Client, Collection, Doc, isId, Markup, MarkupBlobRef, MeasureContext, Mixin, Ref, RefTo, Type, WorkspaceId } from '@hcengineering/core'
...
export class UnifiedConverter {
  ...
  private readonly skipResolveFields = new Set([
    '_class',
    '_id',
    'collection',
    'attachedTo',
  ...
  private async resolveAttribute (
    ...
    if (type._class === core.class.Collection) {
      ...
      return await Promise.all(
        collectionDocs.map(async (doc) => await this.convert(doc))
      )
    }
    ...
    if (type._class === core.class.ArrOf) {
      return await Promise.all(
        (value as any[]).map(async (element) => {
          ...
          return await this.resolveAttribute('', of, element, markdownFields, collabFields, refFields, collectionFields, docId, docClass, attributesOnly)
        })
      )
    }
  ...
  private async resolveAttachments (
    ...
    const resolved = attachments.map((attachment): UnifiedAttachment => ({
      id: attachment._id,
